...
func (c *Cache) Add(response []byte) {
	...
	if err != nil {
		WarnLog("Cache parse response error %v", err)
		return
	}
...
func (c *Cache) loopStore() {
	defer c.waiter.Done()
	InfoLog("Cache store task to %v by %v", c.SaveFile, c.SaveDelay)
	...
	for running {
		...
		case <-ticker.C:
			if c.Update != lastUpdate {
				...
				if err != nil {
					ErrorLog("Cache store cache to %v error %v", c.SaveFile, err)
				}
				lastUpdate = c.Update
			}
		}
	}
	InfoLog("Cache store task to %v is stopped", c.SaveFile)
}
...
func (s *Server) AcquireConn(key string) (conn *Conn, err error) {
	...
	for _, addr := range s.UpperAddr[key] {
		...
		if xerr != nil {
			WarnLog("Server dial connect by %v error %v", addr, xerr)
			continue
		}
...
func (s *Server) procTask(task *requestTask) {
	defer func() {
		if perr := recover(); perr != nil {
			ErrorLog("Server proc task is panic with %v, callstack is \n%v", perr, xdebug.CallStack())
		}
	}()
	response, err := s.Query(task.Request)
	if err != nil {
		WarnLog("Server query error %v\n", err)
		return
	}
...
func (s *Server) procListen(conn net.PacketConn) {
	...
	InfoLog("Server listener on %v is stopped by %v", conn.LocalAddr(), err)
}
